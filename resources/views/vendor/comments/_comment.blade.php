...
<div id="comment-{{ $comment->getKey() }}" class="media">
    ...
    <li id="comment-{{ $comment->getKey() }}" class="media">
        @endif
        @if($comment->commenter instanceof \App\User)
        <a href="{{ route('admin.stu.show' , $comment->commenter) }}" target="_blank" rel="noopener noreferrer">
            <img class="mr-3 rounded-circle" src="https://ui-avatars.com/api/?name={{ $comment->commenter->username }}"
                alt="{{ $comment->commenter->username ?? $comment->guest_name }} Avatar">
        </a>
        @else
        <img class="mr-3 rounded-circle" src="https://ui-avatars.com/api/?name={{ $comment->commenter->username }}"
            alt="{{ $comment->commenter->username ?? $comment->guest_name }} Avatar">
        ...
        <div class="media-body">
            <h5 class="mt-0 mb-1">
                @if($comment->commenter instanceof \App\User)
                <a href="{{ route('admin.stu.show' , $comment->commenter) }}" target="_blank" rel="noopener noreferrer">
                    {{ $comment->commenter->username ?? $comment->guest_name }}
                </a>
                @else
                {{ $comment->commenter->username ?? $comment->guest_name }}
                @endif
